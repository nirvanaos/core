...
inline Pointer WinMemory::allocate (Pointer dst, UWord size, Flags flags)
{
	if (!size)
		throw BAD_PARAM ();
...
inline Pointer WinMemory::copy (Pointer dst, Pointer src, UWord size, Flags flags)
{
	if (!src)
		throw BAD_PARAM ();
...
inline void WinMemory::release (Pointer src, UWord size)
{
	if (!src)
		return;
...
inline void WinMemory::commit (Pointer dst, UWord size)
{
	// Check parameters
...
inline void WinMemory::decommit (Pointer dst, UWord size)
{
	if (!dst)
		throw BAD_PARAM ();
...
inline Word WinMemory::query (Pointer p, Memory::QueryParam q)
{
	switch (q) {
