...
TEST_F (TestMemory, Allocate)
{
	...
	for (int iteration = 0; iteration < ITER_CNT; ++iteration) {
		...
		uint8_t* block = (uint8_t*)ProtDomainMemory::allocate (0, BLOCK_SIZE, flags);
		ASSERT_TRUE (block);
		...
		ASSERT_EQ (block, (uint8_t*)ProtDomainMemory::allocate (block, BLOCK_SIZE, flags));
		...
		ASSERT_EQ (block, (uint8_t*)ProtDomainMemory::allocate (block, BLOCK_SIZE / 2, flags));
...
TEST_F (TestMemory, Commit)
{
	...
	uint8_t* block = (uint8_t*)ProtDomainMemory::allocate (0, BLOCK_SIZE, Memory::READ_WRITE | Memory::RESERVED);
	ASSERT_TRUE (block);
...
{
	size_t BLOCK_SIZE = 0x20000000;	// 512M
	uint8_t* block = (uint8_t*)ProtDomainMemory::allocate (0, BLOCK_SIZE, 0);
	ASSERT_TRUE (block);
	...
	uint8_t* sblock = (uint8_t*)ProtDomainMemory::copy (0, block, BLOCK_SIZE, 0);
	ASSERT_TRUE (sblock);
	...
	EXPECT_EQ (sblock, (uint8_t*)ProtDomainMemory::copy (sblock, block, BLOCK_SIZE, 0));
	EXPECT_EQ (sblock [1], 1);
...
{
	size_t BLOCK_SIZE = 0x20000000;	// 512M
	size_t SHIFT = ProtDomainMemory::SHARING_ASSOCIATIVITY;
	// Allocate block.
...
TEST_F (TestMemory, SmallBlock)
{
	...
	*block = 1;
	...
	ProtDomainMemory::decommit (block, ProtDomainMemory::FIXED_COMMIT_UNIT);
	ProtDomainMemory::commit (block, sizeof (int));
	*block = 1;
	{
		EXPECT_TRUE (ProtDomainMemory::is_private (block, sizeof (int)));
		int* copy = (int*)ProtDomainMemory::copy (0, block, ProtDomainMemory::FIXED_COMMIT_UNIT, Memory::DECOMMIT);
		EXPECT_EQ (*copy, 1);
		EXPECT_TRUE (ProtDomainMemory::is_readable (copy, sizeof (int)));
		EXPECT_TRUE (ProtDomainMemory::is_writable (copy, sizeof (int)));
