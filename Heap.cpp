...
Pointer HeapDirectory::reserve (Pointer heap, UWord cb)
{
  ...
  // Номер блока:
  assert (bitmap_ptr - m_bitmap >= level_bitmap_begin);
...
Pointer Heap::allocate (Pointer p, UWord cb, UWord flags)
{
  assert (cb <= HEAP_SIZE);
  ...
  if (p) {
    ...
    if (end > s + HEAP_PART_SIZE)
      return 0;
    return header () [part].allocate (s, p, cb, flags);
  } else {
    HeapDirectory* dir = header ();
    while (parts) {
      if (parts & 1) {
        Pointer p = dir->allocate (s, 0, cb, flags);
        if (p)
          return p;
      }
      s += HEAP_PART_SIZE;
...
#else
  return header ()->allocate (space (), p, cb, flags);
#endif
}

void Heap::check_allocated (Pointer begin, Pointer end)
{
  assert (end < space_end ());
  assert (begin < end);
  ...
  Octet* s = space ();
  UWord ibegin = (Octet*)begin - s;
  UWord iend = (Octet*)end - s;

#if (HEAP_PARTS > 1)

  UWord part = ibegin / HEAP_PART_SIZE;
  HeapDirectory* dir = header () + part;
  UWord partend = (ibegin & ~(HEAP_PART_SIZE - 1)) + 1;
  ...
  do {
    ...
    if (!dir->check_allocated (ibegin, partend))
      throw BAD_PARAM ();

    ibegin = partend;
    ++dir;
    partend += HEAP_PART_SIZE;
    part >>= 1;
  ...
  if (!header ()->check_allocated (ibegin, iend))
    throw BAD_PARAM ();
