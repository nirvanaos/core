						...
						if (p1) {
							...
							if (MapViewOfFileEx (hm, FILE_MAP_WRITE, 0, 0, 16 * PAGE_SIZE, p1)) {
								if (VirtualAlloc (p1 + PAGE_SIZE, PAGE_SIZE, MEM_COMMIT, PAGE_EXECUTE_WRITECOPY)) {
									...
									ASSERT_EQ (p [5000], 111);
									ASSERT_EQ (p1 [5000], 111);
									p [5000] = 112;
									ASSERT_EQ (p1 [5000], 111);
									p1 [5000] = 113;
									ASSERT_EQ (p [5000], 112);
...
TEST (Test, test3)
{
	...
	ASSERT_EQ (PAGE_EXECUTE_WRITECOPY, mbi.Protect);
}
...
TEST (Test, test5)
{
	...
	ASSERT_EQ (mbi.Protect, PAGE_WRITECOPY);
...
TEST (Test, test6)
{
	...
	ASSERT_EQ (mbi.AllocationProtect, PAGE_EXECUTE_READWRITE);
	...
	VirtualQuery (p1, &mbi, sizeof (mbi));
	ASSERT_EQ (mbi.State, MEM_RESERVE);
...
TEST (Test, test7)
{
	...
	VirtualQuery (p, &mbi, sizeof (mbi));
	ASSERT_EQ (LINE_SIZE * 2, mbi.RegionSize);
	...
	VirtualQuery (p1, &mbi, sizeof (mbi));
	ASSERT_EQ (LINE_SIZE, mbi.RegionSize);
...
TEST (Test, test8)
{
	...
	*/
	...
	VirtualQuery (p, &mbi, sizeof (mbi));
	ASSERT_EQ (mbi.Protect, PAGE_EXECUTE);
