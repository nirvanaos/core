...
{
	ProxyFactory::_ref_type proxy_factory = g_system->bind_interface <ProxyFactory> (primary_iid);
...
void ProxyManager::create_proxy (InterfaceEntry& ie)
{
	if (!ie.proxy) {
		...
		ProxyFactory::_ref_type pf = g_system->bind_interface <ProxyFactory> (iid);
